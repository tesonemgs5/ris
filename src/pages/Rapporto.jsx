...
export default function Rapporto(){
  ...
  function setP(key,val){
    setForm(f=>{
      ...
      const dfmt=gg&&mm&&aa?`${gg}/${mm}/${aa}`:f.p2_data;
      const ofmt=hh&&mi?`${hh}:${mi}`:f.p2_ore;
      if(["data_gg","data_mm","data_aa"].includes(key)){n.p2_data=dfmt;n.data_segn=dfmt;n.data_arrivo=dfmt;n.data_inc=dfmt;n.op_fine_gg=key==="data_gg"?val:f.op_fine_gg;n.op_fine_mm=key==="data_mm"?val:f.op_fine_mm;n.op_fine_aa=key==="data_aa"?val:f.op_fine_aa;}
      if(["ora_hh","ora_mm"].includes(key)){n.p2_ore=ofmt;n.ora_inc_hhmm=ofmt;n.ora_segn_hhmm=ofmt;n.ora_arrivo_hhmm=ofmt;}
